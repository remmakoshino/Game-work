...
🎮 **[今すぐプレイ！](https://remmakoshino.github.io/Game-work/)**
...
- 📱 **マルチプラットフォーム対応** - PC・スマートフォン・タブレットで遊べる
- 📐 **完全レスポンシブ** - あらゆる画面サイズに対応
...
🎮 **https://remmakoshino.github.io/Game-work/**

ブラウザでアクセスするだけですぐにプレイできます！
...
2. **コードをプッシュ**
   ```bash
   git add .
   git commit -m "Update for mobile support and deployment"
   git push origin main
   ```

3. **デプロイの確認**
   - 「Actions」タブで自動デプロイの進行状況を確認
   - 完了後、`https://<username>.github.io/Game-work/` でアクセス可能

### 手動デプロイ

Actionsタブから「Deploy to GitHub Pages」ワークフローを手動実行できます。

### 注意事項

- VRMモデルファイルと音楽ファイルがリポジトリに含まれていることを確認してください
- 大きなファイル（100MB以上）がある場合は、Git LFSの使用を検討してください
- デプロイには数分かかる場合があります
...
### スマートフォン版
1. **メニュー画面**: 「START」ボタンをタップ
2. **楽曲選択**: 好きな楽曲と難易度を選択
3. **ゲーム開始**: 画面をタップしてスタート
4. **プレイ**: 画面下部の **4つのボタン**をタイミングよくタップ
5. **判定**: PC版と同様
6. **リザルト**: スコア確認とリトライ

💡 **ヒント**: スマートフォンでは横向き（ランドスケープ）でのプレイを推奨します！
